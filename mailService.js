...
function getTransporter() {
  ...
  console.log(`📧 [SMTP] host=${host} port=${port} secure=${secure} user=${user || 'MISSING'} pass=${pass ? '***' : 'MISSING'}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// Format a Date object nicely in IST
// ─────────────────────────────────────────────────────────────────────────────
function fmtDate(d) {
  if (!d) return 'N/A';
...
// ─────────────────────────────────────────────────────────────────────────────
// Send email via Nodemailer — returns { sent: true } or throws
// ─────────────────────────────────────────────────────────────────────────────
async function sendMail({ to, subject, html, attachments }) {
  try {
    ...
    // No credentials → log mock only, do NOT throw (fire-and-forget callers)
    if (!config.smtpUser || !config.smtpPass) {
      console.warn(`\n✉️  [MOCK] No SMTP credentials. Email NOT sent.`);
      console.warn(`   To: ${to} | Subject: ${subject}`);
      console.warn(`   → Save Gmail + App Password in Admin > Email Settings\n`);
      return { messageId: 'mock-' + Date.now(), mock: true };
    }

    // Gmail: always use smtpUser as sender (Gmail rejects mismatched From header)
    ...
    await transporter.verify().catch(err => {
      ...
      if (msg.includes('535') || msg.includes('Username and Password not accepted')) {
        ...
        hint = '\n   HINT: Enable 2-Step Verification on Google Account first, then generate an App Password.';
        ...
        hint = '\n   HINT: Connection timed out. Check Railway outbound port 587 is allowed.';
      }
...
function generateInvoicePDF(request, invoice, customer, technician) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.text('Service Center, Kothapet, Nagole, Hyderabad', { align: 'center' });
      doc.moveDown(1.5);
      ...
      doc.text(`Date: ${fmtDate(invoice.createdAt)}`);
      ...
      items.forEach(item => {
        ...
        doc.fillColor('#0f172a').text(`₹${(item.amt || 0).toFixed(2)}`, 450, itemY, { align: 'right' });
        doc.moveDown(0.8);
      });
      ...
      doc.fontSize(12).fillColor('#10b981').text(`₹${(invoice.totalAmount || 0).toFixed(2)}`, 450, totalY, { bold: true, align: 'right' });
      doc.moveDown(1.5);
      ...
      if (invoice.customerSignature && invoice.customerSignature.startsWith('data:image/png;base64,')) {
        try {
          ...
          if (invoice.signedAt) {
            doc.fontSize(8).fillColor('#64748b').text(`Signed at: ${fmtDate(invoice.signedAt)}`, 50);
          }
...
// ─────────────────────────────────────────────────────────────────────────────
// Simulated SMS (console only — integrate Twilio/Fast2SMS for real SMS)
// ─────────────────────────────────────────────────────────────────────────────
...
<head>
  ...
  <style>
    ...
    .header-logo { font-size: 26px; font-weight: 800; letter-spacing: -0.5px; margin-bottom: 4px; }
    .header-tagline { font-size: 12px; color: #93c5fd; text-transform: uppercase; letter-spacing: 1.5px; }
    .header-subtitle { font-size: 13px; color: #bfdbfe; margin-top: 6px; }
    .body { padding: 36px 32px; line-height: 1.65; color: #334155; }
    .body h2 { font-size: 20px; font-weight: 700; color: #0f172a; margin-top: 0; margin-bottom: 18px; }
    .body p { font-size: 15px; margin-bottom: 16px; color: #475569; }
    .details-table {
      width: 100%;
    ...
    .details-table tr:nth-child(even) { background-color: #f1f5f9; }
    .details-table td { padding: 11px 16px; font-size: 14px; border-bottom: 1px solid #e2e8f0; }
    .details-table td.label { font-weight: 600; color: #475569; width: 170px; white-space: nowrap; }
    ...
    .footer .contact-line { font-size: 13px; color: #475569; font-weight: 500; margin-bottom: 6px; }
    .badge {
    ...
    .notice-box p { margin: 0; font-size: 13px; line-height: 1.6; color: #9f1239; font-weight: 500; }
    .btn {
      display: inline-block;
      padding: 13px 30px;
  ...
  <div class="wrapper">
    <div class="container">
      <div class="header">
        ...
        <div class="header-subtitle">7075750640 &nbsp;|&nbsp; 7981785948 &nbsp;|&nbsp; Kothapet, Hyderabad</div>
      </div>
      <div class="body">
        <h2>${title}</h2>
        ${bodyHtml}
      ...
      <div class="footer">
        <p class="contact-line">📞 7075750640 &nbsp;|&nbsp; 7981785948</p>
        <p>✉️ cskelectronicservices@gmail.com</p>
        <p>📍 Service Center, Kothapet, Nagole, Hyderabad</p>
        <p style="margin-top:14px; font-size:11px; color:#94a3b8;">
          This is an automated notification from CSK Electronics Service System.<br>
...
module.exports = { sendMail, generateInvoicePDF, sendSimulatedSMS, buildEmailTemplate, fmtDate };
